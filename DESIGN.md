...
| 층 | z | 예 |
|---|---|---|
| 모달 · 시트 | `z-[100]` | AddMealModal, MealDetailModal |
| 화면 프레임 | `z-50` | TopAppBar, BottomNav |
| 화면 내 고정 바 | `z-30` | `.cal-subbar` (식단 컨트롤) |
| 본문 | auto | 카드, 목록 |

**본문을 `auto` 로 두고 고정 바를 `z-30` 에 두는 건 필수다.** `.eatco-reveal` 이
등장 중 `transform` 을 걸어 카드마다 쌓임 맥락을 만들기 때문에, 고정 바가
`auto` 면 카드가 그 위에 그려진다.

### 상단바 아래 붙는 바

`--eatco-topbar-h` (index.css) 를 쓴다. **`79px` 을 손으로 적지 않는다.**
`top: 79px` 은 설치형 PWA 에서 틀린다 — 상단바가 `env(safe-area-inset-top)` 만큼
더 크므로 그 바가 상단바 뒤로 완전히 숨는다. 반드시
`calc(var(--eatco-topbar-h) + env(safe-area-inset-top))`.

**상시 고정보다 스크롤-오프를 먼저 검토한다.** 설치형 PWA(노치 iPhone 390×844)의
실제 콘텐츠 창은 671px 이 아니라 **614px** 이다. 두 줄짜리 바를 상시로 두면
화면의 43% 가 크롬이 된다. Material 도 이 구성에는 스크롤-오프를 규정한다.
...
- 여백은 `Layout` 이 소유한다. 페이지가 자체 `px-*` 를 덧붙이면 탭 전환 시 콘텐츠 가장자리가
  ...
  **유일한 예외: 가장자리까지 채우는 고정 바.** `-mx-6 px-6` 으로 `Layout` 의 좌우
  패딩을 뚫는다. 안 그러면 24px 거터로 콘텐츠가 바 뒤를 지나가는 게 비쳐 보인다.
  `#root { overflow-x: clip }` 이 뷰포트 밖으로 나간 부분을 잘라주고, `clip` 은
  스크롤 컨테이너를 안 만들어서 `sticky` 도 살아 있다. **이 예외 외에 페이지가
  가로 패딩을 더하는 것은 여전히 금지다.**
