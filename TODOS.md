...
**바뀐 것 (v1.11.0):** 가족이 등록한 키는 호출 수가 보인다(설정 화면).
다만 **앱 공용 키는 여전히 안 보이고**, 키를 등록하지 않은 가족은 아무것도 안 보인다.
절반만 해결됐다.
...
vitest 로도 된다. 그러면 "가족 구성원님이 8월 21일 아침에 계란후라이를 올렸어요" 를
끝에서 끝까지 단언할 수 있다.
...
### 레시피 검토는 아직 가족 키를 안 쓴다 (2026-08-27)

**What:** v1.11.0 의 BYOK 를 `scan.ts`(영수증)에만 연결했다.
`shared-recipes.ts` 의 개선 검토·공개 검토는 여전히 앱 공용 키로 Gemini 를 부른다.

**Why:** 가족이 키를 등록해도 레시피 검토는 앱 쿼터를 계속 먹는다. 사용자 입장에선
"키를 넣었는데 왜 앱 한도에 걸리지" 가 된다. 설정 화면은 「가족이 등록한 키로
영수증을 읽어요」라고 영수증만 말하고 있어서 지금은 거짓말은 아니지만, 절반이다.

**Context:** `leaseKeys` + 폴백 루프를 그대로 옮기면 된다. scan.ts 의 루프가
그대로 본보기다. 지역차단 예외 처리도 같이 가져가야 한다.
...
**What:** 프로덕션 계정 2개(가족 구성원, 다른 구성원)가 모두 `role='admin'` 이다.

**Why:** 관리자는 **모든 가족의 데이터**를 보고 사용자를 지울 수 있다.
가족 구성원이라고 그 권한이 필요한 건 아니다. 다른 구성원님이 의도적으로
