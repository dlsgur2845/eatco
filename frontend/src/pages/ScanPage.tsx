  ...
  const handleCapture = useCallback(async (files: File[], source: ScanSource, notice?: string | null) => {
    ...
    try {
      ...
      if (result.total === 0) {
        ...
        setError(
          result.succeeded === 0
            // 서버가 이유를 말했으면 **그걸 그대로** 보여준다. 지역차단(503)이면
            // 워커가 "AI 기능이 일시적으로 중단됐어요" 라고 정확히 말해주는데,
            // 그걸 버리고 "사진을 읽지 못했어요" 를 띄우면 사진 탓이 아닌 일에
            // 사진을 다시 찍게 만든다. 다시 찍어도 절대 안 된다.
            ? (result.failureDetail ?? '사진을 읽지 못했어요. 다시 시도해주세요.')
